...
export function PendingTasks() {
  ...
  // Obtener solo tareas pendientes asignadas al usuario actual
  const allTasks = getHomeTasks();
  const allPendingTasks = allTasks.filter(
    task => task.assignedTo === currentUser?.id && task.status === 'pending'
  );

  // Filtrar por prioridad
  const pendingTasks = priorityFilter === 'Todas'
    ? allPendingTasks
    : allPendingTasks.filter(task => task.priority === priorityFilter);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
      <div className="container mx-auto py-8 max-w-4xl">
        ...
        <Card title={`📥 Tareas pendientes (${allPendingTasks.length})`} maxWidth="max-w-4xl mx-auto">
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500">
                  {priorityFilter === 'Todas'
                    ? '🎉 No tienes tareas pendientes por aceptar'
                    : priorityFilter === 'Alta'
                    ? '🔴 No tienes tareas pendientes con prioridad Alta'
                    : priorityFilter === 'Media'
                    ? '🟡 No tienes tareas pendientes con prioridad Media'
                    : '🟢 No tienes tareas pendientes con prioridad Baja'}
                </p>
              </div>
            ) : (
