...
import { X, Calendar, RefreshCw, User as UserIcon } from 'lucide-react';
...
export function ReassignTaskModal({
  ...
  const handleReassign = () => {
    ...
    if (!selectedUserId) {
      setError('Debe seleccionar un nuevo responsable.');
      return;
    }
      ...
      >
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-semibold text-gray-800">Reasignar tarea</h2>
          </div>
          <button
        ...
        <div className="mb-6 p-4 bg-gray-50 rounded-lg">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 text-sm text-gray-600">
              ...
              <span>Responsable actual: <strong>{currentResponsible?.name || 'Sin asignar'}</strong></span>
            </div>

            {/* Current Status */}
            <div className="flex items-center gap-2 text-sm text-gray-600">
              <span>Estado actual: <strong>{getStatusBadge(task.status)}</strong></span>
            </div>
          </div>
        </div>

        {/* Select New Responsible */}
        <div className="mb-6">
        ...
        <div className="flex flex-col gap-3">
          <Button onClick={handleReassign} fullWidth disabled={members.length === 0}>
            Confirmar cambio
          </Button>
          {currentResponsible && (
            <Button onClick={handleRemoveAssignment} variant="secondary" fullWidth>
              Quitar asignación
            </Button>
          )}
